...
if( !class_exists( 'YITH_YWRBP_AeliaCS_Module')){
    ...
    class YITH_YWRBP_AeliaCS_Module{
        ...
        public function __construct()
        {
            ...
            add_filter( 'yith_ywrbp_regular_price', array( $this, 'convert_base_currency_amount_to_user_currency'), 10,2 );
            add_filter('yith_ywcrbp_sale_price',array( $this, 'convert_base_currency_amount_to_user_currency'), 10,2 );
        }
        ...
        public function convert_product_role_prices( $product, $currency ){
            ...
            $product_id = yit_get_base_product_id( $product );
            $new_price = YITH_Role_Based_Prices_Product()->get_new_price( $product_id );

            if (  $new_price!=='no_price' ) {
                $product->price = $this->get_amount_in_currency ( $new_price );
            }
            else{
        ...
        public function convert_base_currency_amount_to_user_currency ( $amount, $product_id ) {
            ...
            if ( ! empty( $product->currency ) ) {
                ...
                return self::get_amount_in_currency ( $amount, null, $product->currency );
            }
