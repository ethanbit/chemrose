...
class Tbay_Widget_Recent_Comment extends Tbay_Widget {
    public function __construct() {
        parent::__construct(
            'besa_recent_comment',
            esc_html__('Besa Recent Comments', 'besa'),
            array( 'description' => esc_html__( 'Show list of recent comments', 'besa' ), )
        );
    ...
    public function form( $instance ) {
        if ( isset( $instance[ 'title' ] ) ) {
            ...
            $title = esc_html__( 'Latest Post', 'besa' );
        }
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'number_comment' )); ?>"><?php esc_html_e( 'Num Comments:', 'besa' ); ?></label>
