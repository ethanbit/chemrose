  ...
  $.fn.timeTo = function () {
    ...
    return this.each(function () {
      ...
      if (!data.vals) {
        ...
        $this.addClass('timeTo').addClass('timeTo-' + data.theme).css(css);
        ...
        if (data.displayDays > 0) {
          ...
          for (i = data.displayDays - 1; i > 0; i -= 1) {
            dhtml += i === 1 ? dhtml2 : dhtml2;
          }

          dhtml += dotdays;
          thtml = (data.displayCaptions ? '<figure style="width:' + Math.round(data.width * data.displayDays + marginRight + 4) + 'px">$1' + '<figcaption style="' + fsStyleVal + 'padding-right:' + Math.round(marginRight) + 'px">' + language.days + '</figcaption></figure>' : '$1').replace(/\$1/, dhtml) + thtml;
        }
